...
description: 지금 하고 있는 작업에 가장 잘 맞는 스킬을 자동으로 찾아 쓴다. 이미 이 컴퓨터에 설치된 스킬 중에 맞는 게 있으면 그걸 최우선으로 바로 쓰고(다운로드·검증 불필요라 가장 빠름), 없을 때만 AI Roasting 스킬 라이브러리(Editor's Pick, https://airoasting-skill.vercel.app/?cat=pick)에 큐레이션된 26개 외부 Claude 스킬 카탈로그에서 찾아 추천하고 사용자 승인 시 설치한다. 사용자가 "이 작업에 맞는 스킬 찾아줘", "스킬 추천해줘", "스킬 라이브러리에서 찾아서 적용해줘", "더 잘하는 방법/도구 없을까" 라고 물을 때 반드시 사용한다. 그 외에도 투자·금융 분석, SEO, 광고 계정 감사, 이력서/커리어 문서, 과학·학술 리서치, 디자인 시스템 적용, 글 다듬기(AI 티 제거), GRC/컴플라이언스 점검, 한국 생활 자동화(SRT/카카오톡/미세먼지), 장기 메모리 유지, 에이전트 하네스 구성처럼 카탈로그 카테고리와 맞아떨어지는 작업을 사용자가 시작하면, 명시적으로 요청하지 않아도 먼저 "이 작업에 맞는 스킬이 있는데 써볼까요?" 하고 능동적으로 제안한다. 스킬 설치 전 반드시 출처 검증과 내용 점검을 거친다.
---

# Auto Skill Picker

지금 하려는 작업에 맞는 스킬을 자동으로 찾아 쓰는 스킬이다. 이미 설치된 것부터 확인하고(가장 빠름), 없으면 AI Roasting 스킬 라이브러리의 큐레이션 카탈로그에서 찾아 붙인다.
...
### 1. 먼저 이미 설치된 스킬 중에서 찾는다 (가장 빠르고 안전한 경로)
새로 받아오는 것보다 이미 컴퓨터에 있는 걸 쓰는 게 훨씬 빠르고, 다운로드도 출처 검증도 필요 없다. 그래서 외부 카탈로그를 보기 전에 항상 이것부터 한다:

1. 이 프로젝트의 `.claude/skills/*/SKILL.md`와 사용자 홈 디렉터리의 `~/.claude/skills/*/SKILL.md`를 (Glob 등으로) 찾는다.
2. 각 SKILL.md의 frontmatter(`name`, `description`)만 훑어본다 — 전체 내용을 다 읽을 필요는 없다.
3. 지금 하려는 작업과 잘 맞는 게 있으면, **그걸로 끝이다.** 새로 설치할 필요가 없으니 3~5단계(출처 검증·내용 점검·설치)는 전부 건너뛰고, 바로 "이미 설치된 OO 스킬이 이 작업에 맞으니 그걸로 진행할게" 하고 그 스킬을 쓰면 된다.
4. 이미 설치된 것 중에 마땅한 게 없을 때만 아래 "2. 외부 카탈로그와 매칭"으로 넘어간다.

### 2. 외부 카탈로그와 매칭 (설치된 것 중에 맞는 게 없을 때만)
...
사용자가 후보 중 하나를 고르기 전까지는 어떤 파일도 조회 이상으로 건드리지 않는다. (이 단계부터는 아직 설치 안 된 외부 스킬을 다루는 것이므로 아래 검증 절차가 전부 적용된다 — 1단계에서 이미 설치된 걸 찾았다면 이 단계는 필요 없다.)
